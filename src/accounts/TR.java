...
public class TR {
	...
	private String[] approxCsvCorrection(String[] fields) {
	...
	public TR(String line, BankConfig bc) throws IOException {
		...
		if (bc.getMemoIndex() > -1 && fields.length > bc.getMemoIndex()) {
			String memo = fields[bc.getMemoIndex()].trim();
			if (!memo.isEmpty()) {
				if (getDesc() == null) {
					setDesc(": memo:" + memo + ":");
				} else {
		...
		if (bc.getCheckNoIndex() > -1 && fields.length > bc.getCheckNoIndex()) {
			String checkNoStr = fields[bc.getCheckNoIndex()].trim();
		...
		if (fields.length > bc.getDebitIndex()
				&& !fields[bc.getDebitIndex()].trim().isEmpty()) {
		...
		if (bc.getCreditIndex() > -1 && fields.length > bc.getCreditIndex()
				&& !fields[bc.getCreditIndex()].trim().isEmpty()) {
		...
		if (bc.getFeesIndex() > -1 && fields.length > bc.getFeesIndex()
				&& !fields[bc.getFeesIndex()].trim().isEmpty()) {
